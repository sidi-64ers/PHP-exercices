...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css">    <title>Document</title>
</head>
<body>
    <?php
        $results = array(
        ...
        function Villes($ville){
            ...
            for($i=0; $i < count($ville[0]); $i++){ 
                $element = array_keys($ville[0])[$i];
                echo "<th> $element </th>";
            }   
            echo "</tr></thead><tbody>";
            foreach($ville as $city){ 
                echo $city["ville"] == "PAU" ? "<tr class='z-depth-2 cyan lighten-1'>" : "<tr>";

                for($i=0; $i < count($city); $i++){ 
                    $element = array_keys($city)[$i];
                    echo "
                        <td>$city[$element]</td>                      
                        ";
                }
